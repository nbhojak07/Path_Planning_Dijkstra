# -*- coding: utf-8 -*-
"""ENPM661_Project2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/11htXsQSIJqeH2LrFY27CGMki1RcGufRo
"""
...
def node_is_explored(array):
      list=[]
      p= 0*np.ones((200,300,3))
...
def display_visited(array, im):
     rows, cols =200,300
     vis= node_is_explored(array)
     for i in range(0,rows):
       for j in range(0,cols):
         if (vis[i][j]==(0,255,0)).all():
           im[i][j]=(0,255,0)
     return im 
...
def main():
    ...
    while not visitedAll(arr):
        if (curr_node == goal_node):
            print('found Goal!!')
            tracePath(arr,disp,goal_node)
            break
        arr[curr_node[0]][curr_node[1]].visited = True
        arr = updateNeighbours(arr,map_, curr_node) 
        #display_visited(arr, map_.copy())
        curr_node = findMinCost(arr)
        #p= node_is_explored(arr)
        disp= display_visited(arr, im=map_.copy())
        #cv2_imshow(disp)
